...
namespace WidgetScmDataAccess
{
    public class ScmContext
    {
        ...
        public ScmContext(DbConnection connection)
        {
            ...
            Inventory = connection.Query<InventoryItem>("select * from InventoryItem");
            foreach (var item in Inventory)
            {
                item.Part = Parts.Single(x => x.Id == item.PartTypeId);
            }
            Suppliers = connection.Query<Supplier>("select * from Supplier");
            foreach (var supplier in Suppliers)
            {
                supplier.Part = Parts.Single(x => x.Id == supplier.PartTypeId);
            }
            // ReadParts();
            // ReadInventory();
            // ReadSuppliers();
